...
!*rads_add_ncep -- Add NCEP models to RADS data
!+
...
logical :: dry_on=.false., wet_on=.false., ib_on=.false., air_on=.false., &
	air_plus=.false., error, new=.false.
...
type(model_) :: m1, m2
real(eightbytereal) :: iwv, tmp, slp, dslp
...
if (rads_version ('$Revision$', 'Add NCEP models to RADS data', flag=flag)) return
...
if (new .and. nff(nf90_inq_varid(P%ncid,'dry_tropo_ncep',i)) .and. &
	nff(nf90_inq_varid(P%ncid,'wet_tropo_ncep',i))) then
	write (*,552) 0
	return
endif
...
if (ib_on) call globpres(4,P%equator_time,slp0)
...
!-----------------------------------------------------------------------
get_grids = .true.
if (get_grid(trim(dir)//wet_fmt,hex,model%wet) /= 0) return
if (get_grid(trim(dir)//tmp_fmt,hex,model%tmp) /= 0) return
if (get_grid(trim(dir)//dry_fmt,hex,model%dry) /= 0) return
get_grids = .false.
