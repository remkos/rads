...
\item[\lopt{monthly}] Statistics per calendar month
...
\item[\lopt{format-day}] Output format starts with [YY]YYMMDD (default with \sopt{d} or \lopt{monthly})
\item[\lopt{format-pass}] Output format starts with CYCLE, PASS (default with \sopt{p})
\item[\lopt{groups}] Create groups (applies to NetCDF output only)
\item[\optc{o}{output}{[FILENAME]}] ~\\
...
Normally, the program \prog{radsstat} will be call with one \sopt{S} option. When two are provided, it will compute the difference between collinear missions. For this purpose there are three more command line options:
\begin{description}
\item[\opt{f}{force}] Force comparison, even when missions are not considered collinear
\item[\lopt{first}] Of collinears only provide the first satellite (second editing only)
\item[\lopt{second}] Of collinears only provide the second satellite (first for editing only)
\end{description}
