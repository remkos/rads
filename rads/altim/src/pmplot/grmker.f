...
C+
      SUBROUTINE GRMKER (SYMBOL,ABSXY,N,X,Y,ATTRIB,SIZE,ANGLE)
C
...
* ATTRIB (input, logical): if .TRUE., attributes size and tilt are given by
*       SIZE and ANGLE; if .FALSE., size is the current character size and
*       the tilt is zero.
C--
C (19-Mar-1983)
...
* 18-Jan-1996 - Remko Scharroo: GRSLS removed. Size and tilt and GRCLIPMODE
*               introduced.
C-----------------------------------------------------------------------
      INCLUDE 'grpckg1.inc'
      INCLUDE 'grclip.inc'
      INTEGER  SYMBOL
      INTEGER  C 
      LOGICAL  ABSXY, UNUSED, VISBLE, ATTRIB
      INTEGER  I, K, LSTYLE, LX, LY, LXLAST, LYLAST, N, SYMNUM, NV
      INTEGER  XYGRID(300)
      REAL     ANGLE(*), SIZE(*), COSA, SINA, FACTOR, RATIO, X(*), Y(*)
...
C view surface.
C
      if (grclipmode.ne.1) CALL GRAREA(GRCIDE, 0.0, 0.0, 0.0, 0.0)
C
...
C
      DO 380 I=1,N
          ...
          LYLAST = -64
          IF (ATTRIB) THEN
             COSA = SIZE(I) * FACTOR * COS(ANGLE(I)/57.29577951)
             SINA = SIZE(I) * FACTOR * SIN(ANGLE(I)/57.29577951)
          ENDIF
  320       K = K+2
            LX = XYGRID(K)
            LY = XYGRID(K+1)
...
C         ! negative symbol: filled polygon of radius 8
          ...
          DO 400 I=1,NV
              THETA = 3.141592653*(REAL(2*(I-1))/REAL(NV) + 0.5)
