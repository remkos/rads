...
*
* Specially for degree-minute notation options FORM=3 and FORM=4 are added.
* These will return a string in the format <degrees>\u<minutes>\d.
* Option FORM=5 and FORM=6 allow <hour>:<min> and <hour>:<min>:<sec>
C
C Formatting rules:
...
*       - The input is considered to be an angle in degrees. It is
*         converted to degrees and minutes and plotted as an integer
*         number of degrees and integer number of minutes, written as
*         superscript.
*
...
*       - The input is considered to be time in hours. It is converted
*         to hours and minutes and returned as a string <hours>:<min>
*
...
C                    FORM = 3 -- use degrees-minutes notation always
C                    FORM = 4 -- use degrees-minutes notation if appropriate
C                    FORM = 5 -- use hours:minutes notation
C                    FORM = 6 -- use hours:minutes:seconds notation
...
C  9-Jan-1990 [Remko Scharroo] - Let FORM=1 work for integral numbers.
* 27-Apr-1993 [RS]  - Options FORM=3,4,5,6 included
* 16-Aug-1994 [RS]  - Allign with PGPLOT 4.9h (\\ -> \x)
C-----------------------------------------------------------------------
      CHARACTER*1 BSLASH
      CHARACTER*2 TIMES, UP, DOWN
      CHARACTER*20 WORK, WEXP, TEMP
      INTEGER M, P, ND, I, J, K, NBP, NMAX
      LOGICAL MINUS
C
...
C
      ...
      GOTO (100,100,100,300,300,500,600), FORM+1
*
* FORM=0, 1, or 2 -- Use decimal or exponential
*
  100 CONTINUE
C
...
C
      NMAX=4
C
C If FORM=1 use up to 10 digits [Remko Scharroo].
      IF (FORM.EQ.1) NMAX=10
      IF ((P.GE.0) .AND. (P+ND.LE.NMAX)) THEN
          DO 30 I=1,P
              ND = ND+1
              WORK(ND:ND) = '0'
   30     CONTINUE
...
C
      ...
      RETURN
*
* FORM=3 or 4 -- Use degree-minutes notation
*
  300 continue
*
* M is number of minutes (absolute) and remember if it is negative
*
      M=NINT(MM*10.**PP*60)
      MINUS=(M.LT.0)
      M=ABS(M)
*
* Split M (minutes) in P (degrees) and M (minutes)
*
      P=M/60
      M=M-P*60
      IF (M.EQ.0 .AND. FORM.EQ.4) THEN
         WRITE (WORK,305) P
      ELSE
         WRITE (WORK,310) P,UP,M,DOWN
      ENDIF
  305 FORMAT (I4)
  310 FORMAT (I4,A2,I2.2,A2)
      CALL PGNUMB1(WORK,STRING,MINUS,NC)
      RETURN
*
* FORM=5 -- Use hours:minutes notation
*
  500 CONTINUE
*
* M is minutes. Then split M in minutes (M) and hours (P). Remember if neg.
*
      M=NINT(MM*10.**PP*60)
      MINUS=(M.LT.0)
      P=M/60
  ...
  510 FORMAT (I4,':',I2.2)
      ...
      RETURN
*
* FORM=6 -- Use hours:minutes:seconds notation
*
  600 CONTINUE
*
* I is seconds. Then split I in seconds (I), minutes (M) and hours (P).
*
      I=NINT(MM*10.**PP*3600)
      MINUS=(I.LT.0)
      P=I/3600
      I=I-P*3600
      M=I/60
      I=I-M*60
      WRITE (WORK,610) P,M,I
  610 FORMAT (I4,':',I2.2,':',I2.2)
      CALL PGNUMB1(WORK,STRING,MINUS,NC)
      RETURN
      END

      SUBROUTINE PGNUMB1(WORK,STRING,MINUS,NC)
      CHARACTER*(*) WORK,STRING
      LOGICAL MINUS
      INTEGER NC, L, J, I
      J=0
      L=0
      DO 10 I=1,LEN(WORK)
	 IF (WORK(I:I).NE.' ' .AND. J.EQ.0) J=I
	 IF (WORK(I:I).NE.' ' .AND. J.NE.0) L=I
   10 CONTINUE
      IF (MINUS) THEN
         STRING='-'//WORK(J:L)
         NC=L-J+2
