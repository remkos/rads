...
int openvuf( mainptr *, shallptr *, int *, int * );
...
int main( int argc, char *argv[] )
{
  ...
  if(( argc < 2 ) || ( argc > 3)) {
    ...
    fprintf( stderr, "enter the input and output files\n" );
    exit( 1 );
  }
...
/* and open the tidal data files */
  ...
  if(( cfg = fopen( "tidecor2.4.cfg", "r" )) == NULL ) {
    fprintf( stderr,"Error in opening the configuration file tidecor2.4.cfg\n");
    fprintf( stderr, "Cannot continue!\n\n" );
    exit( 9 );
  }

  dummy = (char *)malloc( 180 * sizeof( char ));
  nodefile = (char *)malloc( 82 * sizeof( char ));
  elefile =  (char *)malloc( 82 * sizeof( char ));

  fgets( dummy, 180, cfg );
  j = sscanf( dummy, "%s", nodefile );
  if( j == EOF || j <= 0 ) {
  ...
  constfile = ( char *)malloc( 82 * sizeof( char ));
  ext = (char *)malloc( 5 * sizeof( char ));
  ext1 = (char *)malloc( 5 * sizeof( char ));

  for( i = 0; i < nconst; i++ ) {
    fgets( dummy, 180, cfg );
    j = sscanf( dummy, "%s", constnames[i] );
    constnames[i] = caseup( constnames[i] );
    if( j == EOF || j <= 0 ) {
    ...
    fgets( dummy, 180, cfg );
    j = sscanf( dummy, "%s", constfile );
    if( j == EOF || j <= 0 ) {
      fprintf( stderr,"Error in reading constituent #%d filename.\n", i );
      fprintf( stderr, "Input line was %s\n", dummy );
      fprintf( stderr, "Cannot continue!\n\n" );
      exit( 9 );
    }
...
/* Open the input/output files. */
  if(( loctimefile = fopen( argv[1], "r" )) == NULL ) {
    fprintf( stderr, "Error in opening the input file!!\n" );
    exit( 1 );
  }
  if(( tidecorfile = fopen( argv[2], "w" )) == NULL ) {
...
/* Read in the constituent data */
  openvuf( cons, shall, &nmain, &nshall );
...
/* Loop through the input file.
  ...
  while(( nread = fscanf( loctimefile, "%lf %lf %ld %ld %ld %ld %lf",
	          ...
	          &minute, &seconds)) != EOF ) {
/*    while(( nread = fscanf( loctimefile, "%lf %lf %ld %ld %ld %ld %ld %lf",
                  &longitude, &latitude, &year, &month, &day,
	           &hour,&minute, &seconds)) != EOF ) {*/
    if( nread != 7 ) {
/*    if( nread != 8 ) {*/
...
/* Ouput the tidal correction */
    if( cplx == 0 ) {
/*    fprintf( tidecorfile,
          "%5.2lf %13.8lf %13.8lf %4ld %3ld %2ld %2ld %5.2lf\n", reslt,
          longitude, latitude, year, dayofyear, hour, minute, seconds);*/
    fprintf( tidecorfile,
          "%7.4lf %13.8lf %13.8lf %4ld %2d %2d %2ld %2ld %2ld %5.2lf\n", reslt,
          longitude, latitude, year,month, day, dayofyear, hour, minute, seconds);
    } else {
/*    fprintf( tidecorfile,
          "%5.2lf %5.2f %13.8lf %13.8lf %4ld %3ld %2ld %2ld %5.2lf\n", reslt,
        reslt2, longitude, latitude, year, dayofyear, hour, minute, seconds);*/
    fprintf( tidecorfile,
          "%7.4lf %7.4f %13.8lf %13.8lf %4ld %2d %2d %2ld %2ld %2ld %5.2lf\n", reslt,
          reslt2, longitude, latitude, year, month, day, dayofyear, hour, minute, seconds);
    }
  }
...
/*****************************************************************************/
int openvuf( mainptr *cons, shallptr *shall , int *nmain, int *nshall )
...
/* Allocate memory for input variables */
  ...
  if(( VUF = fopen( "IOS_tidetbl", "r" )) == NULL ) {
...
/* Check to see if the element side crosses the International Dateline
   (changes sign at +180/-180 degrees) and if so, change one of the
   longitudes so that both endpoints of the side and the point in question
   (ptx,pty) are all the same sign in longitude. */

    if(( xd[i] < -170.0 ) && ( xd[j] > 170.0 )) {
      if( ptx < 0.0 ) {
        xd[j] -= 360.0;
      } else {
        xd[i] += 360.0;
...
/* As above, except for the Greenwich meridian.
   If the side crosses the meridian, make the endpoint on the
   western side of the meridian have a negative longitude. */
    if(( xd[i] < 10.0 ) && ( xd[j] > 350.0 )) {
      xd[j] -= 360.0;
    }
    if(( xd[i] > 350.0 ) && ( xd[j] < 10.0 )) {
      xd[i] -= 360.0;
    }
...
/*  sprintf( wccom, "wc -l %s > lc.dat", nfile );
  system( wccom );
  WC = fopen( "lc.dat", "r" );
  fscanf( WC, "%d", &numnodes );
  fclose( WC );
  sprintf( rmcom, "rm -f lc.dat" );
  system( rmcom );*/
...
/*  sprintf( wccom, "wc -l %s > lc.dat", efile );
  system( wccom );
  WC = fopen( "lc.dat", "r" );
  fscanf( WC, "%d", &numeles );
  fclose( WC );
...
/*      printf( "%d %s: %f %f\n", i, innames[i], ampl[i], phase[i] );
      printf( "f: %f freq:%f vu:%f\n", cons[indx]->f, cons[indx]->freq,
                   cons[indx]->vu );
      printf( "    dthr:%f rev:%f rad:%f\n", dthr, revgmt, radgmt );
      printf( "            res: %f\n", cons[indx]->f * ampl[i] * cos( radgmt ));
*/
