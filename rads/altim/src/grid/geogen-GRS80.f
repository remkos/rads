...
c   
      ...
      character*80 input,output,arg,charscale
...
c      if (lmin.lt.2) lmin=2
      ...
      write(6,551) 'scaling of harmonic coefficients (6.3/7.0)--> '
      read(5,'(a)') charscale
...
* GRS80 ellipsoid
	 ...
	 offset(m) = m*(nmax+1) - m*(m+1)/2 + 1
      ...
      if (charscale(1:1).eq.'y' .or. charscale(1:1).eq.'Y') then
         a_frac=6378136.3d0/ae
         gm_frac=3986004.415d8/gm
         do 888 l=0,maxdeg
         do 888 m=0,l
         cnm(offset(m)+l)=cnm(offset(m)+l)*a_frac**l*gm_frac
         snm(offset(m)+l)=snm(offset(m)+l)*a_frac**l*gm_frac
  888    continue
      endif
