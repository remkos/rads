c
c write Geodyn Binary Residual File to ASCII
c
      implicit none
...
c read header
      read(10) rec20
      write(20,'(6d22.15)') rec20
      nc=nint(rec20(1))
      nsta=nint(rec20(10))

c read GEODYN II unit 5 input
      do 10 i=1,nc
      read(10) char8
      write(20,'(10a8)') char8
   10 continue
...
c read tracking station coordinate records
      do 20 i=1,nsta
      read(10) chrsta,rec8
      write(20,'(a8,8d22.15)') chrsta,rec8
   20 continue
...
c location data
      if (mtype.eq.100) then
         read(10,end=999) rec20(1)
         nm=nint(rec20(1))
         if (nm.gt.1000) then
            backspace(10)
            goto 30
         endif
         write(20,'(d22.15)') rec20(1)
      endif
     ...
     c    .and. mtype.gt.6) then
         ...
         do 70 k1=1,nloc
            ...
            if (kk1.ne.0) then
               ...
               read(10,end=999) (recr(j),j=1,7*nm)
               write(20,'(6d22.15)') (recr(j),j=1,7*nm)
               else
               read(10,end=999) (recr(j),j=1,9*nm)
               write(20,'(6d22.15)') (recr(j),j=1,9*nm)
               endif
            endif
   80       continue
   70    continue
...
c residual data records
      read(10,end=999) (resid(i),i=1,8*nm)
      write(20,'(6d22.15)') (resid(i),i=1,8*nm)
