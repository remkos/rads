...
#define GRIFIL grifil_
#define GROFIL grofil_
#define GRRFIL grrfil_
#define GRWFIL grwfil_
#define GRCFIL grcfil_
#define GRWFCH grwfch_
#define GRSEEK grseek_
#else
#define GRIFIL grifil
#define GROFIL grofil
#define GRRFIL grrfil
#define GRWFIL grwfil
#define GRCFIL grcfil
#define GRWFCH grwfch
#define GRSEEK grseek
#endif

/*
 **&GRIFIL -- Open file for reading with GRFILEIO
 *+
 *     FUNCTION GRIFIL (FNAME)
 *     INTEGER GRIFIL
 *     CHARACTER*(*) FNAME
 *
 * Opens file FNAME for reading.
 * GRIFIL returns the file descriptor for use in subsequent calls to
 * grwfil or grcfil. If GRIFIL is negative, an error occurred while
 * opening the file.
 *
 **
 * Usage:
 *
 *     FD = GRIFIL ('input_file')
 *     CALL GRWFIL (FD, 4, ARRAY)
 *
 * Arguments:
 *  FNAME  (input) : File name of the input file
 *  GRIFIL (output) : Contains the file descriptor on return. If GRIFIL < 0
 *                   an error occurred while opening the file.
 *-
 */
int GRIFIL(fname, fname_len)
     char *fname;
     int fname_len;
{
 ...
 */
    ...
    fprintf(stderr, "grifil: Insufficient memory\n");
  };
  return fd;
}
...
/*
 **&GRRFIL -- GRFILEIO read routine
 *+
 *     FUNCTION GRRFIL (FD, NBYTE, BUFFER)
 *     INTEGER FD, NBYTE, GRRFIL
 *     BYTE    BUFFER(NBYTE)
 *
 * Reads NBYTE bytes from the file associated by descriptor FD (which is
 * returned by the GRIFIL call. The array BUFFER contains the data that has
 * been read, but can (of course) also be associated with any other
 * string, scalar, or n-dimensional array.
 * The function returns the number of bytes actually read in GRRFIL. If
 * GRRFIL < 0, a write error occurred.
 *
 * Arguments:
 *  FD      (input) : File descriptor returned by GRIFIL
 *  NBYTE   (input) : Number of bytes to be read
 *  BUFFER (output) : Buffer containing the bytes that have been read
 *  GRRFIL (output) : Number of bytes read, or (if negative) error code.
 *-
 */
int GRRFIL(fd, nbytes, buf)
     int *fd, *nbytes;
     char *buf;
{
  return read(*fd, (void *) buf, *nbytes);
}
 ...
 *
 * Spools file the the position indicated by the parameter OFFSET (in
 ...
 * Upon return SEEKF is zero, or contains an error code (to be reported by
 * PERRORF).
 *
 * Arguments:
 *  FD     (input) : File descriptor returned by OPENF
 *  OFFSET (input) : Offset in bytes from the point indicated by WHENCE
 *  WHENCE (input) : Indicates whether the OFFSET is counted from the
 *                   start of the file (0), current position (1), or the
 *                   end of the file (2).
 * Returned value:
 *  SEEKF (output) : 0 On proper return, otherwise error code
 *-
 */
int GRSEEK(fd,nbytes,whence)
