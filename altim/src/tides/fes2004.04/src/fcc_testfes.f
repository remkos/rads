...
c   loadlp  : loading effects for long period tides
c  
c   tide+lp             = pure tide (as seen by a tide gauge)
c   tide+lp+load+loadlp = geocentric tide ((as seen by a satellite)
...
c    Initialize memory for FES algorithms
      call fcc_initfes(dir, fes2004, rc)
      if(rc .ne. 0) stop
