...
C Version 6.7RS - 2002 Mar 19
* - Put BoundingBox at top when PGPLOT_PS_BBOX equals "TOP" (R Scharroo)
* - When PGPLOT_PS_COMP is not set or equals "NONE" the images are not
*   compressed
* - When PGPLOT_PS_COMP is set to "CMAP" then a color map is made and the
*   image is coded in these indexes. This compresses color images by almost a
*   factor 3. Gray scale images ARE NOT compressed.
* - When PGPLOT_PS_COMP is set to "RLE" then the images (both color and * B/W)
*   are run-length-encoded. This compresses color images by at least a
*   factor 3. When the image contains only few colors the compression is
*   even bigger.
C
C Supported device: 
...
C bounding-box information in the header of the PostScript file.
C If the variable has a value TOP, PGPLOT will attempt to put the
C correct bounding-box information in the header if the PostScript file.
C (This action mat not be fully platform-independent though). If
...
C
*  PGPLOT_PS_COMP
* When this variable is set to "RLE", images in the file are
* run-length-encoded. This results in a large reduction of the file size,
* especially for color images (at least a factor 3). When the variable is
* set to "CMAP" a color no run-length encoding is performed, but only a
* color map is constructed. This setting has no effect on gray-scale
* images. When the variable is not set or equals "NONE", no compression is
* performed. This is the original PGPLOT mode.
*
...
C
      ...
      REAL     LW
      SAVE     LW
      LOGICAL  NEWCMAP, NEWIMAGE, DORLE, DOCMAP
      SAVE     NEWCMAP, NEWIMAGE, DORLE, DOCMAP
...
C        -- machine-dependent!
 ...
 94         IF (CHR(LCHR:LCHR).EQ.' ') THEN
      ...
      CALL GRPS02(IOERR, UNIT, '%%Creator: PGPLOT [PSDRIV 6.7RS]')
      CALL GRDATE(INSTR, L)
      IF (L.GT.0) CALL GRPS02(IOERR, UNIT,
     :    '%%CreationDate: '//INSTR(1:L))
      CALL GRGENV('PS_BBOX', INSTR, L)
      CALL GRTOUP(INSTR(1:3), INSTR(1:3))
      IF (INSTR(1:3).EQ.'TOP') THEN
* Put a provisionary bounding box (atend) at the top with enough space such that it
* can be replaced at closing by the correct bounding box
         CALL GRPS02(IOERR, UNIT, '%%BoundingBox: (atend)            ')
C        CALL GRPS02(IOERR, UNIT, '%%BoundingBox: 0000 0000 0000 0000')
      ELSE IF (INSTR(1:3).EQ.'MAX') THEN
  ...
  100 CONTINUE
     ...
     1       'writing PostScript file: file is incomplete')
      ...
      IF (.NOT.STDOUT) THEN
         ...
         CALL GRTOUP(INSTR(1:3), INSTR(1:3))
         IF (INSTR(1:3).EQ.'TOP') THEN
            OPEN (UNIT, FILE=FNAME, STATUS='OLD', ACCESS='DIRECT',
     1         RECL=200, FORM='UNFORMATTED')
            READ (UNIT, REC=1) HEAD1
	    LL=INDEX(HEAD1,'%%BoundingBox:')
	    IF (LL.GT.0) THEN
               CALL GRFAO('%%BoundingBox: # # # #', L, INSTR,
     :           BBOX(1), BBOX(2), BBOX(3), BBOX(4))
	       HEAD1(LL:LL+L-1)=INSTR(1:L)
	       WRITE (UNIT, REC=1) HEAD1
	    ENDIF
	    CLOSE (UNIT)
         ENDIF
         CALL GRFLUN(UNIT)
...
C            If this is the first image in this file, define the
C            "cmapimage" or "rlecmapimage" command
          CALL GRGENV('PS_COMP', INSTR, L)
          CALL GRTOUP(INSTR(1:L), INSTR(1:L))
	  DORLE=(INSTR(1:3).EQ.'RLE')
	  DOCMAP=(INSTR(1:4).EQ.'CMAP' .AND. COLOR)
	  IF (.NOT.NEWIMAGE) THEN
          ELSE IF (DORLE .AND. COLOR) THEN
	     CALL GRPS02(IOERR, UNIT,
     |'/rlecmapimage { /buffer 1 string def /rgbval 3 string def'//
     |' /block 384 string def { currentfile buffer readhexstring pop')
	     CALL GRPS02(IOERR, UNIT,
     |'/bcount exch 0 get store bcount 128 ge { 0 1 bcount 128 sub'//
     |' { currentfile buffer readhexstring pop pop')
	     CALL GRPS02(IOERR, UNIT,
     |'/rgbval cmap buffer 0 get 3 mul 3 getinterval store block exch'//
     |' 3 mul rgbval putinterval } for block 0 bcount 127 sub 3 mul'//
     |' getinterval }')
	     CALL GRPS02(IOERR, UNIT,
     |'{ currentfile buffer readhexstring pop pop /rgbval cmap buffer'//
     |' 0 get 3 mul 3 getinterval store 0 1 bcount { block exch 3 mul'//
     |' rgbval putinterval } for')
	     CALL GRPS02(IOERR, UNIT,
     |'block 0 bcount 1 add 3 mul getinterval } ifelse } false 3'//
     |' colorimage } bind def')
          ELSE IF (DORLE) THEN
	     CALL GRPS02(IOERR, UNIT,
     |'/rlecmapimage { /buffer 1 string def /rgbval 3 string def'//
     ...
     |' { currentfile buffer readhexstring pop pop')
	     CALL GRPS02(IOERR, UNIT,
     |'/rgbval cmap buffer 0 get 1 getinterval store block exch'//
     |' rgbval putinterval } for block 0 bcount 127 sub getinterval }')
	     CALL GRPS02(IOERR, UNIT,
     |'{ currentfile buffer readhexstring pop pop /rgbval cmap buffer'//
     |' 0 get 1 getinterval store 0 1 bcount { block exch'//
     |' rgbval putinterval } for')
	     CALL GRPS02(IOERR, UNIT,
     |'block 0 bcount 1 add getinterval } ifelse }'//
     |' image } bind def')
          ELSE IF (DOCMAP) THEN
	     CALL GRPS02(IOERR, UNIT,
     |'/cmapimage { /buffer 1 string def /rgbval 3 string def'//
     |' { 0 1 pcount 1 sub { currentfile buffer readhexstring pop pop')
	     CALL GRPS02(IOERR, UNIT,
     |'/rgbval cmap buffer 0 get 3 mul 3 getinterval store pix exch'//
     |' 3 mul rgbval putinterval } for pix 0 pcount 3 mul getinterval')
	     CALL GRPS02(IOERR, UNIT,
     |'} false 3 colorimage } bind def')
	  ENDIF
	  NEWIMAGE=.FALSE.
C         -- If the colormap was altered after the last dump, (re)define it.
	  IF (.NOT.DOCMAP .AND. .NOT.DORLE) THEN
          ELSE IF (.NOT.NEWCMAP) THEN
	  ELSE IF (COLOR) THEN
	     CALL GRPS02(IOERR, UNIT,
     |'/cmap 768 string def currentfile cmap readhexstring')
             WRITE (UNIT, '((16(3Z2.2,1X)))')
     |(NINT(255*RVALUE(CI)),NINT(255*GVALUE(CI)),NINT(255*BVALUE(CI)),
     |CI=0,255)
	     CALL GRPS02(IOERR, UNIT, 'pop pop')
	  ELSE
	     CALL GRPS02(IOERR, UNIT,
     |'/cmap 256 string def currentfile cmap readhexstring')
             WRITE (UNIT, '((32(1Z2.2,1X)))')
     |(NINT(255*RVALUE(CI)),CI=0,255)
	     CALL GRPS02(IOERR, UNIT, 'pop pop')
	  ENDIF
          NEWCMAP=.FALSE.
...
C         -- 
	  IF (DORLE) THEN
             CALL GRFAO('/pcount # def', L, INSTR, NXP, 0, 0, 0)
	     CALL GRPS02(IOERR, UNIT, INSTR(:L))
	  ELSE IF (DOCMAP) THEN
             CALL GRFAO('/pcount # def', L, INSTR, NXP, 0, 0, 0)
	     CALL GRPS02(IOERR, UNIT, INSTR(:L))
             CALL GRFAO('/pix # string def', L, INSTR, 3*NXP, 0, 0, 0)
             CALL GRPS02(IOERR, UNIT, INSTR(:L))
	  ELSE
	  ...
	  IF (DORLE) THEN
	      CALL GRPS02(IOERR, UNIT, 'rlecmapimage')
	  ELSE IF (DOCMAP) THEN
	      CALL GRPS02(IOERR, UNIT, 'cmapimage')
          ELSE IF (COLOR) THEN
              CALL GRPS02(IOERR, UNIT, 
     ...
     :      '{currentfile picstr readhexstring pop} image')
          END IF
	  CALL GRPS11(UNIT, 0, 0.)	! Initialize image buffers
      ELSE IF (N.EQ.-1) THEN
C         -- Last: terminate image
	  CALL GRPS11(UNIT, -1, 0.)	! Clean image buffers
          CALL GRPS02(IOERR, UNIT, 'grestore')
      ELSE 
C         -- Middle: write N image pixels to the output buffer.
	  IF (DORLE) THEN
	      DO I=1,N
	         CALL GRPS11(UNIT, 2, RBUF(I+1))
	      ENDDO
	  ELSE IF (DOCMAP) THEN
	      DO I=1,N
	         CALL GRPS11(UNIT, 1, RBUF(I+1))
	      ENDDO
	  ELSE
              DO I=1,N
                 CI = RBUF(I+1)
	         CALL GRPS11(UNIT, 1, 255.0*RVALUE(CI))
	         IF (COLOR) THEN
	            CALL GRPS11(UNIT, 1, 255.0*GVALUE(CI))
	            CALL GRPS11(UNIT, 1, 255.0*BVALUE(CI))
	         ENDIF
              ENDDO
	  END IF
      END IF
...
C-----------------------------------------------------------------------
      ...
      SUBROUTINE GRPS11 (UNIT, I, R)
      INTEGER UNIT, I
      REAL    R
*
* Handling image buffers, using RLE (run length encoding) or not
* I=0  : Initialize buffer
* I=1  : Add 'pixel' (colorindex J=NINT(R)) to buffer (no encoding)
* I=2  : Add 'pixel' (colorindex J=NINT(R)) to buffer (RLE encoding)
* I=-1 : Close down buffer (return to clean state)
*
      integer j,jold,nbuf,nrun,buf(128)
      save    j,jold,nbuf,nrun,buf

      j=nint(r)
      if (i.eq.0) then
	 call GRPS12(UNIT,  0,buf)
	 nrun=1
	 nbuf=0
      else if (i.eq.-1) then
         if (nrun.gt.1) then		! We have some run-length left over
	    call GRPS12(UNIT, 1,nrun-1)
	    call GRPS12(UNIT, 1,jold)
	 else if (nbuf.ge.1) then	! We have some non run-length left over
	    call GRPS12(UNIT, 1,nbuf+127)
	    call GRPS12(UNIT, nbuf,buf)
	 endif
	 call GRPS12(UNIT, -1,buf)
	 nrun=1
	 nbuf=0
      else if (i.eq.1) then
         call GRPS12(UNIT, 1,j)
      else
         if (nbuf.eq.0 .or. j.ne.jold) then	! Last two pixels differ.
	    if (nrun.gt.1) then	! If we have some runlength,
	       call GRPS12(UNIT, 1,nrun-1)
	       call GRPS12(UNIT, 1,jold)
	       nrun=1
	       nbuf=0
	    endif
	 else			! Last two pixels are the same.
	    if (nbuf.gt.nrun) then	! If we have some non-run length
	       call GRPS12(UNIT, 1,nbuf-nrun+127)
	       call GRPS12(UNIT, nbuf-nrun,buf)
	       buf(1)=j
	       nrun=1
	       nbuf=1
	    endif
	 ...
	 nbuf=nbuf+1
	 buf(nbuf)=j
	 if (nrun.eq.128) then	! If buffer is full with runlength
	    call GRPS12(UNIT, 1,nrun-1)
	    call GRPS12(UNIT, 1,jold)
	    nrun=1
	    nbuf=0
	 else if (nbuf.eq.128) then	! If buffer is full with non-run
	    call GRPS12(UNIT, 1,nbuf+127)
	    call GRPS12(UNIT, nbuf,buf)
	    nrun=1
	    nbuf=0
      ...
      subroutine GRPS12(UNIT, l,map)
      integer UNIT, l,n,i,map(l),buf(60)
      save n,buf
      data n/0/
      if (l.eq.0) then
         n=0
      else if (l.lt.0) then
         write (UNIT,600) (buf(i),i=1,n)
	 n=0
      else
         do i=1,l
            n=n+1
