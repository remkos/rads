c
c     conversion of prare data to geodyn II binary format
c
c Created by Pieter Visser for Metric format
c Adapted for gbf format by Eelco Doornbos [05-04-2002]
c
...
* GBF variables
      ...
      real*4    sigma,trop,meteo,riono,displ,cmas
      real*8    tfrac,range,date,date1,deltat,co

      integer*4 idata(17)
      equivalence
     |  (idata(1),satid), (idata(2),mtyptim),(idata(3),statid),
     |  (idata(4),prepro),(idata(5),mjd),    (idata(6),tfrac),
     |  (idata(8),range), (idata(10),dummy1),(idata(11),dummy2),
     |  (idata(12),sigma),(idata(13),trop),  (idata(14),meteo),
     |  (idata(15),riono), (idata(16),displ), (idata(17),cmas)

* PRARE variables

      integer*4 nsatel, jahr, itag, it1, it2, numsta, icamp, inpt, nrang
      integer*4 ictro2, iono, icion2, massz2, iphsz2, mccgs2, iexca2, i91val
      integer*4 if1, if2, if3, if4, if5, if6, if7, if8, if9, if10, isdmes
      real*8 sec, azim, elev, range2, press, temp, wett

* Other variables

      real*8 secday,freqtr0,dfrac,rmul,timsec,vlight,difft
      integer*4 i,icur,ipre,nmulti,nobs,nonpra,nutc,mdate
      parameter(secday=86400d0)
      parameter (freqtr0=8489d6)
      integer*4 phasz2,grouz2,gropz2,if(10),it(2),ipreh(31)
      integer*4 imout(12),mjd0(0:99)
      character reckey*8,if11*1,input*60,output*60
      real*8 xutc(2000),tutc(2000),rmout(19)
      real*8 rmntro,rmnion,rmncom,rmnphs,rmngro,rmngrp
      real*8 rmstro,rmsion,rmscom,rmsphs,rmsgro,rmsgrp
...
* Fill in the gbf elements
     
      satid      = nsatel
      mtyptim(1) = 28     ! PRARE Range
      mtyptim(2) = 03     ! Ground received / UTC time
      statid     = numsta
      prepro     = 0
      prepro     = prepro + 2**(31-10)         ! Met word present

*      if(if1.eq.1.or.if1.eq.3.or.if1.eq.5)
*     |prepro     = prepro + 2**(31-12)         ! Tropo corr. not applied
*      if(if2.eq.1.or.if2.eq.3.or.if2.eq.5.or.if2.eq.7.or.if2.eq.9) 
*     |prepro     = prepro + 2**(31-13)         ! Iono corr. not applied
*      if(if3.eq.1)
*     |prepro     = prepro + 2**(31-3)
      ...
      dummy2     = 0
      sigma      = 0.0
      trop       = ictro2*1d-12*vlight/2d0 ! Tropospheric refraction correction
      meteo      = 0.0
      riono      = icion2*1d-12*vlight/2d0 ! Ionospheric refraction correction
      displ      = 0.0
      cmas       = 0.0
